...
public class ApplicationInitializer implements ApplicationRunner {
    ...
    /**
     * Checks if the permissions are available. The permissions are required to set
     * the domain based access control and should be imported through the existing
     * flyway migrations.
     */
    private void checkExistenceOfPermissions() {
        ...
        if (permissionsLength == 0) {
            log.fatal(
                "Couldn't find any permissions in the database. This will most likely lead " +
                    "to errors. Please ensure flyway is enabled in your application.yaml! " +
                    "Alternatively the permissions may be inserted manually - even if " +
                    "not recommended!");
        }
    }
}
